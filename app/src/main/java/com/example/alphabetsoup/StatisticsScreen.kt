...
fun StatisticsScreen(onBack: () -> Unit) {
    ...
    Scaffold(
        ...
        if (records.isEmpty()) {
            ...
            val grouped = records
                .groupBy { ConfigKey(it.size, it.difficulty, it.useDiagonals, it.useSecondHints) }
                .entries
                .sortedWith(compareBy({ it.key.size }, { it.key.difficulty.ordinal }))
                .map { it.toPair() }
            ...
            LazyColumn(
                ...
                items(grouped) { (key, solves) ->
                    ConfigCard(key, solves)
                }
...
@Composable
private fun ConfigCard(key: ConfigKey, solves: List<SolveRecord>) {
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(
            ...
            Text(configLabel(key), fontWeight = FontWeight.SemiBold, fontSize = 15.sp)
...
private fun HistoryItem(record: SolveRecord) {
    Row(
        ...
        Column {
            Text(
                configLabel(ConfigKey(record.size, record.difficulty, record.useDiagonals, record.useSecondHints)),
